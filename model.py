...
class CDAutoEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.detach()
        #################################################### Add noise##############################################
        x_noisy = x * (Variable(x.data.new(x.size()).normal_(0, 0.1)) > -.1).type_as(x) # this is called mask noise to use the original lossless input as the target.
...
class StackedAutoEncoder_MNIST(nn.Module):
    r"""
    A stacked autoencoder made from the convolutional denoising autoencoders above.
    Each autoencoder is trained independently and at the same time.
    """
    ...
    def __init__(self):
        super(StackedAutoEncoder_MNIST, self).__init__()
        
        self.ae1 = CDAutoEncoder(1, 128, 2) #MNIST: one channel
        # self.ae1 = CDAutoEncoder(3, 128, 2)
        self.ae2 = CDAutoEncoder(128, 256, 2)
        self.ae3 = CDAutoEncoder(256, 512, 2)
